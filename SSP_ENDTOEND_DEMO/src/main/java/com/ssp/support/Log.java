...
//			testMachine = "(Browser: " + ((RemoteWebDriver) driver).getCapabilities().getBrowserName() + ", Hub: " + hub + ", Node: " + ")";
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (Reporter.getCurrentTestResult().getTestContext().getCurrentXmlTest().getName().equalsIgnoreCase("UXP_WJ"))
			params[1] = params[1].toString().split("\\(")[0];  
...
//			extentedReport.log(LogStatus.FAIL, eMessage + extentedReport.addScreenCapture(screenShotFolderPath +inputFile));
	 ...
	 *             - java/selenium exception
	 */
	public static void exception(AssertionError e, WebDriver driver, ExtentTest extentedReport) throws Exception {

		String inputFile = takeScreenShot(driver);
		String eMessage = e.getMessage();

		failsoft(e.getMessage());
		
		if (eMessage != null ) {
			eMessage = eMessage.split(" expected ")[0].toString();
		}
...
//			extentReporterTest.log(LogStatus.INFO, passmsg);
	...
	public static void softAssertThat(boolean status, String passmsg, String failmsg, WebDriver driver, ExtentTest extentedReport, Boolean... takeScreenShot) {
        ...
        if (finalDecision) {
            inputFile = takeScreenShot(driver);
            getScreenShotHyperLink(inputFile);
            Reporter.log(PASS_HTML_BEGIN + passmsg+ PASS_HTML_END1 + PASS_HTML_END2);
        }
        else{
            Reporter.log(FAIL_HTML_BEGIN + failmsg + FAIL_HTML_END1 + FAIL_HTML_END2);
