...
public class WaitUtils {
  ...
  public static int maxPageLoadWait = 80;
  ...
  static {
    ...
    spinnerLoad = new ExpectedCondition<Boolean>() {
      public final Boolean apply(final WebDriver driver) {
         ...
         * spinners = driver.findElements(allSpinners); for (WebElement spinner : spinners) { try {
         * if (spinner.isDisplayed()) { return false; } } catch (NoSuchElementException ex) {
         * ex.printStackTrace(); } }
         */
        return true;
      }
  ...
  public static void waitForSpinner(final WebDriver driver) {
    try {
      ...
      Log.event("Catched spinner load exception or Spinner is taking lot of time");
    }
    Log.event("Spinner Load Wait: (Sync)");
  }
  ...
  public static boolean waitForDisabledElement(WebDriver driver, WebElement element, int maxWait) {
    ...
    try {
      ...
      Log.event("Unable to find disabled element after " + " sec ==> " + element.toString());
    }
  ...
  public static boolean waitForElement(WebDriver driver, WebElement element, int maxWait) {
    ...
    try {
      ...
      statusOfElementToBeReturned = false;
      Log.event("Unable to find a element after" + element.toString());
    }
    return statusOfElementToBeReturned;
  }
  ...
  public static void waitForPageLoad(final WebDriver driver, int maxWait) {
    ...
    try {
      ...
      /*
       * wait.until(WebDriverFactory.imagesLoad); wait.until(WebDriverFactory.framesLoad);
       */
  ...
  public static void waitForElementPresent(WebDriver driver, WebElement elementTocheck,
      String msg) {
    (new WebDriverWait(driver, 200).pollingEvery(200, TimeUnit.MILLISECONDS)
        .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
        .withMessage(msg)).until(ExpectedConditions.visibilityOf(elementTocheck));
  }
  ...
  public static void waitForinvisiblityofElement(WebDriver driver, int seconds, WebElement element,
      String msg) {
    (new WebDriverWait(driver, seconds).pollingEvery(200, TimeUnit.MILLISECONDS)
        .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
        .withMessage("Unable to Switch to Windows"))
  ...
  /*
   * public static void waitForinvisiblityofElement_element(WebDriver driver, int seconds,String
   * locator, String msg){ List<WebElement> Elements_toFind =
   * driver.findElements(By.cssSelector(locator)); (new WebDriverWait(driver, 180).pollingEvery(200,
   * TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class,
   * StaleElementReferenceException.class).withMessage(msg)).until((ExpectedConditions.
   * invisibilityOfElementLocated(Elements_toFind)));
   * 
   * }
   */
}
