...
public class ManageContactDetailsPage extends LoadableComponent<ManageContactDetailsPage> {
	...
	@FindBy(css = "#C2__C1__QUE_753975C04A0241C1630604")
	WebElement selectAddAddress;
	...
	public boolean verifyPostCodeErrorMsg(String msg) {
		...
		if (WaitUtils.waitForElement(driver, errmsgPostCode)) {
			if (Utils.verifyWebElementTextEquals(errmsgPostCode, msg)) {
				status = true;
			}
		}
		return status;
	...
	public String getPostCodeAddressFromList() {
		return Utils.getTextOfWebElement(lstPostCodeDetail, driver);
	}
	...
	public boolean verifyFullAddressPostCode(String msg) throws Exception {
		boolean status = false;
		String fullAddress = Utils.getTextOfWebElement(txtFullAddress, driver);
		
		if (fullAddress.contains(msg)){
			status = true;
		}
		return status;
	}
	 ...
	 * @throws Exception
	 */
	public void enterNewAddressDetails(String postcode,HashMap<String, String> testData, String type, ExtentTest extentedReport, boolean Screenshot)
			throws Exception {
		try {
		 ...
		 /*if((testData.get("Brand Name").equals("Westbrom"))){
			 (new WebDriverWait(driver, 20).pollingEvery(200, TimeUnit.MILLISECONDS)
						...
						.withMessage("Timed out to find 'Upgrade policy' dropdown"))
								.until(ExpectedConditions.elementToBeClickable(selectAddAddress));

				Select upgradepolicydropdown = new Select(selectAddAddress);
				upgradepolicydropdown.selectByValue("1");
				WaitUtils.waitForSpinner(driver);
				WaitUtils.waitForElement(driver, btnAddAddressSave);
				}*/
	...
	public void enterAddress(String postCode, String type)
		...
		try {
			...
			Thread.sleep(2000);
			Utils.getMatchingTextElementFromList(drpoptAddAddressTypeOptions, type).click();
