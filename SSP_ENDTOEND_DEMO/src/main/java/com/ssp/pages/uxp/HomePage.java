...
public class HomePage extends LoadableComponent<HomePage> {
	...
	public String cssBtnCancel = "#C2__C1__p4_BUT_874EFCFEFA062832841943>div";
	...
	@FindBy(css = "img[alt='Asda Business Unit ']")
	WebElement imgAsdaBU;

	@FindBy(css = "img[alt='The West Brom Business Unit']")
	WebElement imgWestbrom;

	@FindBy(css = "input[title='AllBrands']")
	WebElement btnAllBrands;

	@FindBy(css = "a[button-id*='BUT_874EFCFEFA062832841943']")
	WebElement mdlMyBrndsCloseBtn;

	@FindBy(css = "#C2__C1__p4_BUT_874EFCFEFA062832841943>div")
	...
	@FindBy(css = "#C1__p1_HEAD_C99AAF40EE7EDAD4122375")
	...
	protected void isLoaded() {
		...
		(new WebDriverWait(driver, 10).pollingEvery(200, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class)
	...
	public void clickChangePassword(ExtentTest extentedReport) throws Exception {
		WaitUtils.waitForElement(driver, cmbUserDrpDwnForLogout, 10);
		cmbUserDrpDwnForLogout.click();
	...
	public String getAppVersionInAbtPopUp() {
		return Utils.getTextOfWebElement(txtAbtECVersion, driver);
	}
	...
	public String getAppLangInAbtPopUp() {
		return Utils.getTextOfWebElement(txtAbtECLang, driver);
	}
	...
	public String getAppTitleInAbtPopUp() {
		return Utils.getTextOfWebElement(txtAbtPopUpTitle, driver);
	}
	...
	public String getLoginUserName() {
		return Utils.getTextOfWebElement(cmbUserDrpDwnForLogout, driver);
	}
	...
	public void clickMyBrands(String brandname, ExtentTest extentedReport) throws Exception {
		...
		if (brandname.contains("ASDA")) {
			WaitUtils.waitForElement(driver, imgAsdaBU);
			imgAsdaBU.click();
			status = true;
		} else if (brandname.contains("Westbrom")) {
			WaitUtils.waitForElement(driver, imgWestbrom);
			imgWestbrom.click();
			status = true;
	...
	public boolean verifyBrandsPopup() throws Exception {
		boolean status = false;
		try {
			if (WaitUtils.waitForElement(driver, imgAsdaBU) && WaitUtils.waitForElement(driver, imgWestbrom)) {
				status = true;
			}
		} catch (Exception e) {
	...
	public void clickBrandPopupCloseBtn(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		...
		if (WaitUtils.waitForElement(driver, mdlMyBrndsCloseBtn)) {
			...
			WaitUtils.waitForinvisiblityofElement(driver, 30, cssBtnCancel, "Brand popup did not close after waiting for 30sec");
	...
	public void clickOutsideBrandPopup(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		...
		if (WaitUtils.waitForElement(driver, lnkTakeCall, 3)) {
			...
			WaitUtils.waitForinvisiblityofElement(driver, 60, cssBtnCancel, "Brand popup did not close after waiting for 30sec");
			WaitUtils.waitForelementToBeClickable(driver, titleMyDashboard, "Unable to find outside popup element");
	...
	public boolean verifyMyDashboardTitle() throws Exception {
		return Utils.verifyWebElementTextEquals(titleMyDashboard, "My Dashboard");
	...
	public boolean verifyBrandsInPopup(String brandName, ExtentTest extentedReport, boolean screenshot)
			throws Exception {
		boolean status = false;
		if (WaitUtils.waitForElement(driver, imgWestbrom) && WaitUtils.waitForElement(driver, imgAsdaBU))
