...
public class SearchPage extends LoadableComponent<SearchPage> {
    ...
    @FindBy(css = "#C2__p1_HEAD_7C37BC9733C00CA7665728")
	...
	uielement = new com.ssp.support.ElementLayer(driver);
    }
	...
	if (!Utils.verifyWebElementTextContains(er_wrongSearch, textToVerify)) {
	    return false;
	}
	return true;
    }
	...
	if (!Utils.verifyWebElementTextContains(erPolicyNotExist, textToVerify)) {
	    return false;
	}
	return true;
    }
	...
	WaitUtils.waitForElement(driver, txtPolicyNumber, 20);
	if (!Utils.verifyWebElementTextContains(er_lnameNotFound, textToVerify)) {
	    return false;
	}
	return true;
    }
	...
	WaitUtils.waitForElement(driver, txtPolicyNumber, 20);
	if (!Utils.verifyWebElementTextContains(er_fnameNotFound, textToVerify)) {
	    return false;
	}
	return true;
    }
	...
	try {
	    //WaitUtils.waitForElementPresent(driver, 10, txtPolicyNumber, "Timed out to find policy number text box.");
	...
	//WaitUtils.waitForElementPresent(driver, 10, txtPolicyNumber, "Timed out to find search button on search page.");
	btnSearch.click();
	WaitUtils.waitForSpinner(driver);
	...
	try {
	    WaitUtils.waitForElementPresent(driver, 10, contact_Table, "Timed out to get search results");
     ...
     */
    /*
     * public HashMap<String, String> enterJointCustomerDetails(HashMap<String,
     ...
     * shouldForceRandomNameCreation[0] : false; String title1 =
     * testData.get("Title").toString(); String firstName1 =
     * testData.get("First Name").toString(); String lastName1 =
     * testData.get("Last Name").toString(); String dob1 =
     * testData.get("Date of Birth").toString(); String gender1 =
     * testData.get("Gender").toString(); String postcode1 =
     * testData.get("Post Code").toString(); String homephone1 =
     * testData.get("Home Phone").toString(); String email1 =
     * testData.get("Email").toString();
     * 
     * Log.message("Entering Customer details"); selectTitle(title1);
     ...
     * 
     * entercn_LastName(lastName1); testData.put("Last Name", lastName1);
     * Log.message("Last Name : " + lastName1, extendReport);
     * 
     * entercn_DateOfBirth(dob1); Log.message("Date of Birth : " + dob1,
     * extendReport);
     * 
     * selectGender(gender1); Log.message("Gender : " + gender1, extendReport);
     * 
     * entercn_PostCode(postcode1); Log.message("Post Code : " + postcode1,
     * extendReport);
     * 
     * selectAddress(extendReport); // this feature of selectingf the address is
     * changed // To handle an AJAX request which is processed after the
     * selection of 'Find // Address'
     * 
     * testData.put("Address1", getAddress1()); testData.put("Address2",
     * getAddress2()); testData.put("Address3", getAddress3());
     * testData.put("Postcode", postcode1);
     * 
     * entercn_HomePhone(homephone1); Log.message("Home Phone : " + homephone1,
     * extendReport);
     * 
     ...
     * 
     * Log.message("Email : " + email1, extendReport); testData.put("Email",
     * email1);
     * 
     * selectRadioButton(radPreferences, "Email");
     * 
	...
	return Utils.verifyWebElementTextEquals(txt_searchresHeading, "Select a policy/quote to verify");
	...
	WaitUtils.waitForElementPresent(driver, 10, txt_withnopolicies,
		"Unable to find message 'There are no Quotes or Policies associated with this Contact' on search page");
	try {
	    return Utils.verifyWebElementTextContains(txt_withnopolicies, SEARCH_NOPOLICY_MSG);
	} catch (Exception e) {
	    throw new Exception(
	...
	WaitUtils.waitForElementPresent(driver, 10, btn_proceed, "Unable to find proceed button on search page");
	try {
	    btn_proceed.click();
	    WaitUtils.waitForSpinner(driver);
	...
	try {
		...
		tradename_str = Utils.getRandomCharacters("alpha", 6);
	...
	try {
		...
		WebElement RowVal = driver.findElement((By.cssSelector(specific_policy_Cnt)));
		if (Utils.verifyWebElementTextContains(txt_PolicyNo, policyNo))
		...
		if (WaitUtils.waitForElement(driver, table_policysearchsingleResult, 1)
			&& !Utils.verifyWebElementTextContains(txt_withnopolicies, SEARCH_NOPOLICY_MSG)) {
		    boolean boolstatus = false;
		    if (Utils.verifyWebElementTextContains(txtcontactName, cntctName))
	...
	try {
	    String address1 = Utils.getTextOfWebElement(txtfldAddress1, driver);
	...
	try {
	    return Utils.getTextOfWebElement(txtfldAddress2, driver);
	...
	try {
	    return Utils.getTextOfWebElement(txtfldAddress3, driver);
    ...
    public void selectAddedParty(String row, boolean screenShot, ExtentTest extentedReport) throws Exception {
	try {
	    String specific_Cnt = "#C2__FMT_C47EBF5E7217CB164249167_R2";
	    WebElement RowVal = driver.findElement((By.cssSelector(specific_Cnt)));
	    WaitUtils.waitForElement(driver, RowVal);
	    RowVal.click();
	    WaitUtils.waitForSpinner(driver);
	    Log.message("Selected a valid contact", driver, extentedReport, true);

	    String contactRow = "#C2__p0_TBL_9C3BD3DB194DC120259818_R" + row;
	    WebElement Row = driver.findElement((By.cssSelector(contactRow)));
	    Row.click();
