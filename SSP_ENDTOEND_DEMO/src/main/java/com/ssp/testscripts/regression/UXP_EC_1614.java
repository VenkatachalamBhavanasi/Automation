...
public class UXP_EC_1614 extends BaseTest {
	...
	public void TC_831_832_833_836_838_835(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.message("Naviagated to " + testData.get("tabToClick") + " page successfully", driver, extentedReport,
						true);
	...
	//@Test(description = "Verify that user able to update Payment Plan / Method details in policy renewal,Check that the Manage Renewals and Renewal Quote Variation tabs are no longer be available on the Customer Dashboard, when the quote variation is bought and verify the policy status changes to Renewed", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_843_845_853(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				testData2.put("Payment Plan", "PCL ASDA");
				testData2.put("Payment Method", "Premium Finance");
								
				newquotepage.selectPayment(testData2, true, extentedReport);	
				buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Reviewed", "QuoteDescription", "Save", extentedReport);
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(newquotepage.verifyPayorName(payorname),
						...
						"The user is not able to change the payor of the policy, once the quote reached Acceptance page : "
								+ payorname,
						driver, extentedReport, true);
				Log.softAssertThat(!buyQuotePage.verifyAcceptButton(extentedReport), "Accept button is disabled by default as expected",
				...
				Log.softAssertThat(buyQuotePage.verifyAcceptButton(extentedReport), "Accept button isn't enabled after entering transaction id",
						"Accept button is enabled as expected after entering transaction ID", driver, extentedReport, true);
	...
	public void TC_839_840(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				/*buyQuotePage.upgradePolicy(extentedReport);
				buyQuotePage.verifyUpgraded();
...
*/
	...
	@Test(description = "Check the warning message when lapse the renewal quote variation from Acceptance page and check that renewal quote variation is set as a lapse", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check that the renewal quote is saved as a new variation with a status of Reviewed, when it is saved from Acceptance page", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check that the user is allowed to update the User Preferences details", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_841(String browser) throws Exception {
		...
		String correspondencePref = "Email";
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				newquotepage.clickEditPreferences(correspondencePref, extentedReport, true);
				...
				Log.softAssertThat(custdashboardpage.verifyPreferences(correspondencePref, extentedReport, true),
						"Customer dashboard page has the selected preferences during quote renewal, Preference : " + correspondencePref,
						"Customer dashboard page failed to have selected preferences during quote renewal, Preference : " + correspondencePref, driver, extentedReport, true);
	...
	public void TC_848_849_850_851(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						hmQuotedetails.get("quoteStatus_0").toString().contentEquals("R")
						&& hmQuotedetails.get("quoteStatus_1").toString().contentEquals("L"),
						"Quote variations in customer dashboard page are listed as Lapsed after clicking lapse from acceptance page",
						"Quote variations in customer dashboard were not listed as Lapsed after clicking lapse from acceptance page", driver, extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = "Verify that user able to complete payment for a lapsed quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void BS_014(String browser) throws Exception {
		...
		try {
			...
			// Navigate to Login Page
			LoginPage loginPage = new LoginPage(driver, webSite,extentedReport).get();
			Log.message("SIAAS Landing Page : " + webSite, driver, extentedReport, true);
			...
			*/
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				newquotepage.clickAgree(extentedReport);
				newquotepage.clickGetQuote(extentedReport);
				newquotepage.clickRenewalQuoteBuy(extentedReport, "Renewal");
				...
				HashMap<String, String> hmQuotedetails = custdashboardpage.getQuoteVariations(extentedReport, true);
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("L")) {
					Log.message("Quote variations in customer dashboard page are listed as lapsed", extentedReport);
					...
					*/
					...
					if(custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
									custdashboardpage))
						Log.pass("Customer Dashboard page verified and user able to complete payment transaction for lapsed renewal quote", driver, extentedReport, true);
					else
						Log.fail("Customer Dashboard page not verified", driver, extentedReport, true);
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("L")) {
					...
					Log.softAssertThat(buyQuotePage.verifyAcceptButton(extentedReport), "Accept button is enabled as expected after entering transaction ID",
							"Accept button isn't enabled after entering transaction id", driver, extentedReport, true);
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("R")
					...
					Log.message("Quote variations in customer dashboard page are listed as Review required and Lapsed",
							extentedReport);
				else
				...
				if(policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")
								&& policyDetails.get("Position").toString().equalsIgnoreCase("Renewal"))
					Log.pass("Renewal Policy Created Successfully in accepted status", driver, extentedReport, true);
						
				else
					Log.fail("Failed to create Renewal Policy, it stands in following Status and position stands :"
							+ policyDetails.get("Status").toString() + policyDetails.get("Position").toString(), driver, extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "Verify that more renewal variations can be viewed in the quote variations dropdown and review the icons displayed and perform renewal for the original created quote variation(a lapsed quote)", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(groups = "Business_Scenario", description = "To check user able view the renewal quote in a contact search and if multiple quotes, all are visible and  view the status of the renewal quote in a contact search", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(searchPage.verify_reviewStatus("Review Required", extentedReport, true),
						"Review Required status was displayed after saving a renewal quote-Test Completed",
						"Review Required status was not displayed after saving a renewal quote-Test Completed", driver,
						extentedReport);
			}
	...
	@Test(groups = "Business_Scenario", description = "To check user Accessing a policy within the renewal period will have auto-quote review and auto-quote may be lapsed in manage renewals", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", dependsOnMethods = "BS_001_002")
	public void BS_003_004(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(custdashboardpage.verifyOptionInManageRenewals("Lapse Quote", extentedReport, true),
					"Lapse Quote is displayed in Manage Renewal-Test Completed", "Lapse Quote is not displayed in Manage Renewal-Test Completed",
					driver, extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "To check user able to select an existing quote and create a quote variation and second is lapsed and user able to view the active and lapsed quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", dependsOnMethods = "BS_003_004")
	public void BS_005_006_007_008_013(String browser) throws Exception {
		...
		try {
			...
			buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Review Required", quoteDescription2, "Save",
					extentedReport);
			...
			Log.softAssertThat(buyQuotePage.uielement.verifyPageElements(Arrays.asList("txtTitle"), buyQuotePage),
					"Able to view the lapsed quote variation-Test Completed", "Not able to view the lapsed quote variation-Test Completed", driver,
					extentedReport, true);
	...
	public void BS_009_010_011_012(String browser) throws Exception {
		...
		try {
			...
			homePage = custdashboardpage.clickCompleteButton();
			Log.softAssertThat(homePage.verifyHomePage(),
					"User able to complete the quote variations and navigated to Home page.-Test Completed ",
					"User is not able to complete the quote variations and not navigated to Home page. -Test Completed", driver,
					extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = "Check that quote variation is not able to save without discription", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(groups = "Business_Scenario", description = "Verify that Reviewed quote variation have green R in quote variation, Verify that Review Required quote variation have red R in quote variation, Verify that Lapsed quote variation have blue L in quote variation", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").equals("R") && hmQuotedetails.get("quoteStatus_1").equals("L")
					...
					Log.message("Renew Quote variations in customer dashboard page are listed as Reviewed and lapsed",
							extentedReport);
				} else {
	...
	@Test(groups = "Business_Scenario", description = "Check that lapsed quote variation is not able to delete, Verify that alert message should present when lapse all the variation", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").equals("R")) {
					...
					if(buyQuotePage.verifyWarningMsg(extentedReport, warningMsg))
						Log.pass("Alert message is present after clicked the lapse button", driver, extentedReport, true);
					else
						Log.fail("Alert message is not present after clicked the lapse button", driver, extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = " To check Reinstate a cancelled policy, quote then quote variation. Cycle through quote variations until completing. For DD (at renewal and mid-term) and card", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(groups = "Business_Scenario", description = "Check that user can able to complete lapsed varition by reinstate it", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				*/
				...
				if(custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails") ,custdashboardpage))
					Log.pass("User is able to complete lapsed varition by reinstate it as expected", driver, extentedReport, true);
				else
					Log.fail("Validation Failed - User is unable to complete lapsed varition by reinstate it", driver, extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = "check that user can able to use the variations dropdown to lapse specific variations", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(groups = "Business_Scenario", description = "Verify that two contacts produce exactly the same quote with same conditions and cover", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				*/
				...
				if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
					...
					buyQuotePage = new BuyQuotePage(driver, extentedReport);
					buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Reviewed", renewQuoteDesc, "Save", extentedReport);
					...
					if (amountForSecondContact.equals(amountForFirstContact)) {
						Log.pass("Payment amount is same for two persons", driver, extentedReport, true);
					} else {
	...
	//Not in Scope for ASDA
	//@Test(groups = "Business_Scenario", description = "Verify that user not able to complete quote variation without confirm the gurarantee when using Direct debit", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(groups = "Business_Scenario", description = " Not be able to save a quote as reviewed if the quote fails any of the policy validations", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void BS_035(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if(!custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),custdashboardpage))
					Log.pass("Unable to save the renewal quote as Reviewed when there is claims added to the policies", driver, extentedReport, true);
				else
					Log.fail("Able to save the renewal quote as Reviewed when there is claims added to the policies", driver, extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = "verify the premium is same at pp and customer dashboard page for renewal. Premium is displayed at customer dashboard after saving policy in review required status", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void BS_123(String browser) throws Exception {
