	...
	public void TC_58(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(NewQuotePage.verifyPolicyHolderSection(title + " " + testData.get("First Name") + " " + testData.get("Last Name"), extentedReport),
						"Policy holder section displayed the customer name",
						"Policy holder section failed to display customer name in NewQuotePage");
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (BoolVal != false) {
					...
					Log.softAssertThat(!NewQuotePage.verify_searchButton(extentedReport),
					...
					NewQuotePage.selectContactType(testData.get("AddInterestContactType"), extentedReport);
...
//				}
					...
					NewQuotePage.checkaddedInterestedParty(extentedReport, testData.get("Last Name"), true);
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				// Loop to add items to the existing ins cover
				 for (int i = 0; i < coversToAdd.length; i++) {
					...
					Log.softAssertThat(NewQuotePage.verify_defaultContactTab("Personal",extentedReport,true), "Personal contact tab was selected by default",
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				// Loop to add items to the existing ins cover
				 for (int i = 0; i < coversToAdd.length; i++) {
					...
					Log.softAssertThat(NewQuotePage.verify_defaultContactTab("Corporate",extentedReport,true), "Corporate contact tab was set to default tab",
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				// Loop to add items to the existing ins cover
				 for (int i = 0; i < coversToAdd.length; i++) {
					...
					NewQuotePage.Click_contactTabs("Corporate",extentedReport,true);
					
					Log.softAssertThat(NewQuotePage.verify_defaultContactTab("Corporate",extentedReport,true), "User able to switch between personal and corporate contact tabs",
