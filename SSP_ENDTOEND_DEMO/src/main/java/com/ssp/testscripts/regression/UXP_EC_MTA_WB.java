...
public class UXP_EC_MTA_WB extends BaseTest {
	...
	public HashMap<String, String> getTestData(String testcaseId) {
		...
		String className = "UXP_EC_MTA_" + env + "_WB";
	...
	public void TC_005(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					custdashboardpage.verifyTransactionStatus(expectedPosition, expectedTransactionStatus,
							extentedReport, true),
					"MTA transaction is completed successfully and displayed under Transactions tab ",
					"Failed - MTA transaction is not completed and not displayed under Transactions tab - Test case completed",
					driver, extentedReport, true);
			...
			Log.softAssertThat(
					custdashboardpage.verifyTransactionStatus(expectedPosition1, expectedTransactionStatus1,
							extentedReport, true),
					"Verified that the Cancellation transaction status is dispalyed under Transactions tab",
			...
			Log.softAssertThat(
					custdashboardpage.verifyTransactionStatus(expectedPosition2, expectedTransactionStatus2,
							extentedReport, true),
					"Verified that the Reinstatement status is dispalyed under Transactions tab - Test case completed",
	...
	public void TC_006(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					custdashboardpage.verifyTransactionStatus(expectedPosition, expectedTransactionStatus,
							extentedReport, true),
					"MTA on the Reinstated Policy is completed successfully - Test case completed",
	...
	public void TC_013(String browser) throws Exception {
		...
		try {
			...
			newquotepage.clickConfirmBtnInMTA(extentedReport);
			
			Log.softAssertThat(
					custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"), custdashboardpage),
					"Navigated to customer dashboard page after 2nd MTA is completed - Test case completed",
					"Failed - Not navigated to customer dashboard page after 2nd MTA is completed - Test case completed", driver, extentedReport,
					true);
	...
	public void TC_014(String browser) throws Exception {
		...
		try {
			...
			// Get excess value
			String displayedExcess = newquotepage.getExcessValue(ins_RowtoInteract, cover[0], testData);	
			int valuetochk = Integer.parseInt(sepecifiedValue[0]) + Integer.parseInt(sepecifiedValue[1]);
	...
	public void TC_002(String browser) throws Exception {
		...
		try {
			...
			/*Log.softAssertThat(AdjustmentAmount.equals(Premium),
					Tax + " Correct Premium value is displayed in transaction table",
					"Failed - " + Premium + " Premium value is not displayed in transaction table");*/
			...
			Log.softAssertThat(custdashboardpage.verifyPayableInTransactionTable(Tax, Premium, Payable),
					...
					"Failed - " + Payable
							+ " Payable value is not displayed in transaction table - Test case completed", driver, extentedReport, true);
	...
	public void TC_009(String browser) throws Exception {
		...
		try {
			...
			String jointPolicyName2 = jointPolicyFirstName + " " + jointPolicyLastName;
			...
			Log.softAssertThat(
					...
					"Joint policyholder1 is created with FirstName and LastName - " + custDetails2.get("First Name") + " "
							+ custDetails2.get("Last Name"),
					"Joint policyholder1 is not created with FirstName and LastName - ", driver, extentedReport, true);
			...
			Log.softAssertThat(
					...
					"Joint policyholder2 is created with FirstName and LastName - " + custDetails3.get("First Name") + " "
							+ custDetails2.get("Last Name"),
					"Joint policyholder2 is not created with FirstName and LastName - ", driver, extentedReport, true);
			searchPage.clickComplete(extentedReport);
			...
			Log.softAssertThat(
					...
					"Joint policyholder3 is created with FirstName and LastName - " + custDetails4.get("First Name") + " "
							+ custDetails4.get("Last Name"),
					"Joint policyholder3 is not created with FirstName and LastName -", driver, extentedReport, true);
			searchPage.clickComplete(extentedReport);			
			...
			Log.softAssertThat(
					custdashboardpage.verifyContactName(testData.get("Title") + " " + custDetails.get("First Name")
							+ " " + custDetails.get("Last Name"), extentedReport, true),
					"Main policyholder is created with FirstName and LastName -" + custDetails.get("First Name") + " "
							+ custDetails.get("Last Name"),
					"Main policyholder is not created with FirstName and LastName -", driver, extentedReport, true);
			...
			// close joint policyholder popup
			newquotepage.clickCancelBtnInJointPolicyHolderPopup(extentedReport);
			
			// To click yes to remove joint policy
			newquotepage.clickYesToRemovePolicy(jointPolicyName2, true, extentedReport);
			Log.softAssertThat(!newquotepage.verifyPolicyHolderContactName(jointPolicyName2, true, extentedReport),
					jointPolicyName2 + " - Joint Policyholders is removed",
					jointPolicyName2 + " - Joint Policyholders is not removed", driver,
					extentedReport, true);
			newquotepage.clickReCalculate(extentedReport);
			...
			Log.softAssertThat(ins_RowtoInteract == null, "Contents Accidential Damage cover is not displayed",
		...
		try {
			...
			for (int i = 0; i < coversToAdd.length; i++) {
				...
				if (BoolVal != false) {
					...
					Log.fail("Failed to " + coverType[0] + "_" + coverType[1] + " cover is not added", driver,
							extentedReport, true);
				}
			...
			Log.softAssertThat(
					custdashboardpage.verifyTransactionStatus(expectedPosition, expectedTransactionStatus,
							extentedReport, true),
					"MTA as 'Inactive' status is dispalyed and Quote is saved - Test case completed",
	...
	public void TC_004(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(adjustmentAmount.equals(changeToPolicyTermPremium),
					"Adjustment amount and Change to Policy term premium is same in trascation section",
					"Failed - Adjustment amount and Change to Policy term premium is not same in trascation section", driver, extentedReport, true);
			...
			Log.softAssertThat(
					custdashboardpage.verifyTransactionStatus(expectedPosition2, expectedTransactionStatus2,
							extentedReport, true),
					expectedPosition2 + " - status is found in transaction table - Test case completed", 
					"Failed - " + expectedPosition2 + "  status is not found in transaction table - Test case completed",
					driver, extentedReport, true);
 ...
 @Test(description = "Do MTA by changing 3 to 5 star and update data capture page", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
    public void TC_003(String browser) throws Exception {
		...
		Log.softAssertThat(
				newquotepage.uielement.verifyPageElements(
						Arrays.asList("lnkLimitsInBuildings", "drpVoluntaryBuildExcess"), newquotepage),
				"Building cover popup contains Volunatry Excess and limits",
				"Failed - Building cover popup does not contains Volunatry Excess and limits", driver, extentedReport, true);
        ...
        Log.message("Old Premium : " + beforeNewPolicyTermPremium + " ,New Premium :" + NewPolicyTermPremium1, extentedReport);

        Log.softAssertThat(
                newquotepage.verifyNewPolicyTermPremiumChange(beforeNewPolicyTermPremium,
                        NewPolicyTermPremium1, "Increased"),
                "Premium got incresed after adding the cover : " + contents1[0],
                "Failed - Premium not increased after adding the cover : " + contents1[0], driver, extentedReport, true);
			...
			if (BoolVal2 != false) {
				Log.pass(coverType[2] + " " + coverType[0] + "_" + coverType[1] + " cover type done successfully",
				  ...
				  newquotepage.clickReCalculate(extentedReport);
		            ...
		            Log.message("Old Premium : " + beforeNewPolicyTermPremium + " ,New Premium :" + NewPolicyTermPremium2, extentedReport);
