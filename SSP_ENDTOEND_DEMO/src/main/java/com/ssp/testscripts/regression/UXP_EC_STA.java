...
public class UXP_EC_STA extends BaseTest {
	...
	public HashMap<String, String> getTestData(String testcaseId) {
		...
		String className = "UXP_EC_STA_" + env;
	...
	public void TC_013(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(newquotepage.uielement.verifyPageElements(Arrays.asList("btnReCalculate"), newquotepage),
					...
					"Re-calculate button is not displayed at the bottom of the Policy Adjustment page");
			...
			Log.softAssertThat(accidentalExcess.equals(testData.get("BLD_ExcessAmt").toString()),
					"Same Excess Value is displayed for Accidental Damage which was entered for buildings - Test case completed",
					"Same Excess Value is not displayed for Accidental Damage which was entered for buildings - Test case completed", driver,
					extentedReport, true);
	...
	public void TC_018(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(newquotepage.uielement.verifyPageElements(Arrays.asList("btnReCalculate"), newquotepage),
					"Re-calculate button is displayed at the bottom of the Policy Adjustment page - Test case completed",
					"Re-calculate button is not displayed at the bottom of the Policy Adjustment page - Test case completed");
			newquotepage.clickReCalculate(extentedReport);
	...
	@Test(description = "To add cover type High Risk Items", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_021_022_023(String browser) throws Exception {
		// Get the web driver instance
		String tcId = "TC_021_022_023";
		final WebDriver driver = WebDriverFactory.get(browser);
		HashMap<String, String> testData = getTestData(tcId);
		...
		try {
			...
			// Open High Risk Pop up
			String[] coversToAdd = testData.get("coverToSelect").toString().split(",");
			boolean BoolVal = false;
			String ins_RowtoInteract = newquotepage.SelectInsuranceItem(coversToAdd[0], true, extentedReport);
			//newquotepage.clickBuildingsEditButton("High Risk Items", false);

			Log.softAssertThat(newquotepage.verifyHighRiskItemPopupText(),
					newquotepage.MSG_HIGH_RISK_ITEMS_POPUP + " : Message is displayed in the high risk item popup",
					newquotepage.MSG_HIGH_RISK_ITEMS_POPUP + " : Message is not displayed in the high risk item popup",
					driver, extentedReport, true);

			Log.softAssertThat(
					newquotepage.uielement.verifyPageElements(Arrays.asList("btnYesNOHighRiskItems"),
							newquotepage),
					"High Risk Popup is displayed with Yes Or No button",
					"High Risk Popup is not displayed with Yes Or No button", driver, extentedReport, true);
			newquotepage.clickHighRiskYesOrNoButton("Yes");

			Log.softAssertThat(
					newquotepage.verifyMininumAndMaximumSum("Minimum")
							&& newquotepage.verifyMininumAndMaximumSum("Maximum"),
					"Maximum Sum is 1500 and Minimum Sum is 7500 are displayed in High Risk Item Popup",
					"Maximum Sum is 1500 and Minimum Sum is 7500 are displayed in High Risk Item Popup", driver,
					extentedReport, true);
			...
			Log.softAssertThat(BoolVal, "Limit field for HRItem displayed the sum of all the itemvalue provided",
					"Limit field for HRItem not displayed the sum of all the itemvalue provided", driver,
					extentedReport, true);
			
			Log.softAssertThat(
					newquotepage.uielement.verifyPageElements(
							Arrays.asList("btnRemoveHighRisk", "btnEditHighRisk"), newquotepage),
					"Remove and Edit button is displayed for High Risk after adding it - Test case completed",
					"Remove and Edit button is not displayed for High Risk after adding it - Test case completed", driver, extentedReport,
					false);
	...
	@Test(description = "Check if adjustment amount increases after adding covers(STA) in the policy adjustment page - Technology and Entertainment", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	public void TC_024_025_026(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					newquotepage.uielement.verifyPageElements(
							Arrays.asList("btnEditGardenCover", "btnRemoveGardenCover"), newquotepage),
	...
	public void TC_030_031_032_033(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					newquotepage.uielement.verifyPageElements(
							Arrays.asList("btnEditPersonalItem", "btnRemovePersonalItem", "btnReCalculate"), newquotepage),
			...
			Log.softAssertThat(savedDetail.get("Cover").equals(valueToUpdate),
					"User is able to edit the saved value in Personal Item popup - Test case completed",
					"User is able to edit the saved value in Personal Item popup - Test case completed", driver, extentedReport,
					true);
...
NewQuotePage.selectNumberofBedrooms("4");
	...
	public void TC_098(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"), custdashboardpage),
					"User can perform an STA when the previous STA is in IR status - Test case completed",
					"User can not perform an STA when the previous STA is in IR status - Test case completed", driver, extentedReport, true);
	...
	public void TC_130(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(financialDetailAfter.get("AdjustmentAmount").equals(financialDetailAfter.get("ChangeToPolicyTermPremium")),
					"Adjustment amount and Change to Policy term premium are same - Test case completed",
					"Adjustment amount and Change to Policy term premium are same - Test case completed", driver, extentedReport, true);
