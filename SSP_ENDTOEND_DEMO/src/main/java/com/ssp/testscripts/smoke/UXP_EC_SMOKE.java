...
public class UXP_EC_SMOKE extends BaseTest {
	...
	public void TC_001(String browser) throws Exception {
		...
		Log.testCaseInfo("EC-ASDA-TC_001" + description + "<small><b><i>[" + browser + "]</b></i></small>");
		...
		try {
			...
			Log.softAssertThat(newquotepage.verifyNewQuotePage(true, extentedReport), "Verified NewQuotePage ",
					"Not verified NewQuotePage", driver, extentedReport);
	...
	public void TC_003(String browser) throws Exception {
		...
		try {
			...
			for (int i = 0; i < coversToAdd.length; i++) {
				...
				if (BoolVal != false) {
					Log.message(coverType[2] + " " + coverType[0] + " cover done successfully", driver, extentedReport,
							true);
				} else {
		...
		try {
			...
			for (int i = 0; i < coversToChck.length; i++) {
				...
				if (!(valToReturn.equalsIgnoreCase("false"))) {
					...
					Log.message("New quote Page has the default values as expected for " + coverSec + " cover,Limit : "
							+ Exp_limit_Amt + ", Excess:" + Exp_Acess_Amt, driver, extentedReport, true);
				} else {
					Log.fail(
							"New quote Page has failed to populate with following default values for " + coverSec
									+ " cover,Limit : " + Exp_limit_Amt + " , Excess : " + Exp_Acess_Amt,
			...
			for (int i = 0; i < coversToAdd.length; i++) {
				...
				if (BoolVal != false) {
					 ...
					 * else { Log.fail("Failed to " + coverType[2] + " " +
					 * coverType[0] + " cover", driver, extentedReport, true); }
					 */
			}
			// Click on recalculate
			newquotepage.clickReCalculate(extentedReport);
	...
	public void TC_011(String browser) throws Exception {
		...
		try {
			...
			if (custdashboardpage.verifyContactAddressDetails(address)) {
				Log.pass("System displayed newly mailing address under Contact Detail section : " + address, driver,
						extentedReport, true);
			} else {
				Log.fail("System not displayed newly mailing address under Contact Detail section : " + address, driver,
						extentedReport, true);
			}
	...
	// 1599
	// @Test(description = "Check that clicking on 'Yes' option in Remove
	// Interested Party' modal removes the Interested Party with policy but
	// retains the Contact", dataProviderClass = DataProviderUtils.class,
	...
	public void TC_014(String browser) throws Exception {
		...
		//	homePage.clickOutsideBrandPopup(extentedReport, true);
			Log.message("Make  Call - Close button , Cancel button and outside pop up clicks works as expected",
					extentedReport);
	...
	public void TC_015(String browser) throws Exception {
		...
		final WebDriver driver = WebDriverFactory.get(browser);
		HashMap<String, String> testData = getTestData(tcId);
		// HashMap<String, String> customerDetails1 = new HashMap<String,
		// String>();
		...
		try {
			...
			if (NewQuotePage.verifyBinIcon(jointPolicyName, false, extentedReport)) {
				Log.pass("Remove joint Policyholder feature is accessable for the user (Bin icon is present)", driver,
						extentedReport, true);
			} else {
				Log.fail("Remove joint Policyholder feature is not accessable for the user (Bin icon is not present)",
						driver, extentedReport, true);
			}
	...
	public void TC_016(String browser) throws Exception {
		...
		try {
			...
			if (!newquotepage.verifyPolicyHolderContactName(jointPolicyName, true, extentedReport)) {
				Log.pass(jointPolicyName + " - is removed using remove Joint Policyholders feature", driver,
						extentedReport, true);
			} else {
				Log.fail(jointPolicyName + " - is not removed using remove Joint Policyholders feature", driver,
						extentedReport, true);
			}
	...
	// 1607
	// @Test(description = "Check that 'Reverse Transaction' option is available
	// under the 'Manage Policy' sub-section after performing MTA in EC
	// application and verify Modal", dataProviderClass =
	...
	public void TC_017(String browser) throws Exception {
		...
		try {
			...
			if (newquotepage.uielement.verifyPageElements(Arrays.asList("mdlReverseTransaction", "btnConfirmReversal"),
					newquotepage)) {
	...
	public void TC_018(String browser) throws Exception {
		...
		try {
			...
			// Navigate to Login Page
			LoginPage loginPage = new LoginPage(driver, webSite, extentedReport).get();
			Log.message("UXP SIAAS Landing Page : " + webSite, driver, extentedReport, true);
	...
	// 1753 & 1754
	// @Test(description = "Verify User not able to login with Invalid UserName
	// and Password and valid UserName and Invalid Password and verify password
	// screen", dataProviderClass = DataProviderUtils.class, dataProvider =
	// "ssBVTDataProvider")
	...
	// 1614
	// @Test(description = "Verify Reverse transaction after doing Invite
	// Renewal", dataProviderClass = DataProviderUtils.class, dataProvider =
	// "ssBVTDataProvider")
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						...
						"No change in the policy inception / expiry date.Renewed Inception Date:"
								...
								+ polDet_AfterRevTran.get("ExpiryDate"),
						driver, extentedReport, true);
				if (polDet_AfterRevTran.get("InceptionDate").equalsIgnoreCase(policyDetails.get("InceptionDate"))
						&& polDet_AfterRevTran.get("ExpiryDate").equalsIgnoreCase(policyDetails.get("ExpiryDate"))) {
					Log.pass("Inception Date and Expiry date reversed successfully. Renewed Inception Date:"
							+ policyDetailsRnwl.get("InceptionDate") + ", Renewed Expiry Date:"
							+ policyDetailsRnwl.get("ExpiryDate") + ".Reveresed Inception Date:"
							+ polDet_AfterRevTran.get("InceptionDate") + ",Reveresed Expiry Date:"
							+ polDet_AfterRevTran.get("ExpiryDate"), driver, extentedReport, true);
				} else {
					Log.fail("No change in the policy inception / expiry date.Renewed Inception Date:"
							+ policyDetailsRnwl.get("InceptionDate") + ", Renewed Expiry Date:"
							+ policyDetailsRnwl.get("ExpiryDate") + ".Reveresed Inception Date:"
							+ polDet_AfterRevTran.get("InceptionDate") + ",Reveresed Expiry Date:"
							+ polDet_AfterRevTran.get("ExpiryDate"), driver, extentedReport, true);
	...
	// @Test(description = "Create NB (Add interested party) and do Main search
	// for IP and see whether the search results shows the Policy details",
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				custdashboardPage = searchPage.clickSearchedContactPolicy("policy", policyDetails.get("PolicyNo"), "",
						true, extentedReport);
				Log.message("Policy details displayed for the added interested party", extentedReport);
				custdashboardPage.verifyCustomerDashboardPage();
				Log.message("Navigated to Cutomer Dashboard", driver, extentedReport, true);
				...
				if (custdashboardPage.verifyContactName(CustomerName, extentedReport, true)) {
					Log.pass("Verified Interested party FirstName and LastName on Customer Dashboard", driver,
							extentedReport, true);
				} else {
					Log.fail("Not Verified Interested party FirstName and LastName on Customer Dashboard", driver,
							extentedReport, true);
				}
	...
	// @Test(description = "Corporate Contact Creation, Replace address for
	// Corporate contact ", dataProviderClass = DataProviderUtils.class,
	// dataProvider = "ssBVTDataProvider")
	public void TC_024(String browser) throws Exception {
		...
		try {
			...
			if (custdashboardPage.verifyEditBusinesAddress(extentedReport, true)) {
				Log.pass("Replaced address reflects in customer dashboard page", driver, extentedReport, true);
			} else {
				Log.fail("Failed to replace the address in customer dashboard page", driver, extentedReport, true);
			}
	...
	// @Test(description = "MTA using Single Bill Payment (for NB Monthly DD
	// Policy)", dataProviderClass = DataProviderUtils.class, dataProvider =
	// "ssBVTDataProvider")
	public void TC_025(String browser) throws Exception {
		...
		try {
			...
			if (custdashboardpage.verifyTransactionStatus(expectedPosition, expectedTransactionStatus, extentedReport,
					true)) {
				Log.pass("Verified that the MTA transaction status is '" + expectedTransactionStatus + "' as expected",
						driver, extentedReport, true);
			} else {
				Log.fail("The MTA transaction status is not as expected", driver, extentedReport, true);
			}
	...
	// @Test(description = "Verify Billing Adjustment", dataProviderClass =
	// DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_026(String browser) throws Exception {
		...
		try {
			...
			if (custdashboardpage.verifyPaymentMethod(paymentMethod)) {
				Log.pass("Billing Adjustment is completed - Payment method is changed to - " + paymentMethod, driver,
						extentedReport, true);
			} else {
				Log.fail("Billing Adjustment is not completed - Payment method is not changed to - " + paymentMethod,
						driver, extentedReport, true);
			}
		...
		try {
			...
			if (policyDetails.get("Status").equalsIgnoreCase("Accepted")) {
				...
				if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
					...
					Log.message("MTA Policy Created Successfully in accepted status", driver, extentedReport, true);
				...
				//TODO Work of Purendra. Comment the below line
				...
				if (complaintPage.ECbannertitleCheck(extentedReport)) {
					...
					Log.fail("Complaint page failed to have banner title as Engagement Centre", driver, extentedReport,
							true);
				}
	...
	// @Test(description = "Check that the user is able to complete the
	// suspension by selecting Accept button", dataProviderClass =
	// DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_028(String browser) throws Exception {
		...
		try {
			...
			SuspendBillingPage suspendBillingPage = custdashboardpage
					.selectSuspendBillingfromManagePolicy(extentedReport, true);
			...
			UnsuspendBillingPage unsuspendBillingPage = custdashboardpage
					.clickUnsuspendBillingfromManagePolicy(extentedReport, true);
	...
	// 1699
	// @Test(description = "Check whether the default policy flags will be set
	// as Automatic Review - Checked,Automatic Renewal - Checked,Block renewal -
	// Unchecked and check the flags after selecting block renewal and verify
	// that changes remains when user perform MTA again", dataProviderClass =
	// DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_029(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (NewQuotePage.validateReviewRenewalFields("Block Renewal", "checked", extentedReport, true)) {
					Log.pass("Block Renewal field is checked as expected", driver, extentedReport, true);
				} else {
					Log.fail(
